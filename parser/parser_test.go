...
func TestParser_ProgramAddNums(t *testing.T) {
	...
	if !bytes.Equal(instructionCode, []byte{0, 0, 8, 0, 0, 5, 4, 46}) {
		t.Errorf("Expected tos to be '0, 0, 8, 0, 0, 5, 4, 46' error message but was %v", instructionCode)
	}
}
...
func TestParser_ProgrammFunctionCall(t *testing.T) {
	...
	if !bytes.Equal(instructionCode, []byte{0, 1, 217, 228, 0, 0, 5, 21, 11, 2, 46, 27, 0, 27, 1, 4, 23}) {
